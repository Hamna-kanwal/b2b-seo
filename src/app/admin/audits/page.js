      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
          <h1 style={{ 
            ...
            Admin Panel - SEO Audit Requests
          </h1>
        ...
        {loading ? (
          <p style={{ color: '#6b7280', textAlign: 'center', padding: '20px', fontSize: '14px', fontWeight: '500' }}>Loading audit requests...</p>
        ) : audits.length === 0 ? (
          <div style={{ backgroundColor: '#ffffff', padding: '35px', borderRadius: '24px', border: '1px solid rgba(121, 40, 202, 0.2)', boxShadow: '0 10px 30px rgba(121, 40, 202, 0.06)', textAlign: 'center' }}>
            <p style={{ color: '#6b7280', margin: 0, fontSize: '15px', fontWeight: '500' }}>No SEO audit requests received yet.</p>
          </div>
        ) : (
              ...
              <table style={{ width: '100%', minWidth: '700px', borderCollapse: 'collapse', color: '#1f2937' }}>
                <thead>
                  <tr style={{ background: 'linear-gradient(135deg, #7928CA 0%, #9333EA 100%)', color: '#ffffff', textAlign: 'left' }}>
                    ...
                    <th style={{ padding: '14px 16px', fontSize: '13px', fontWeight: '700', minWidth: '220px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Context</th>
                  </tr>
                </thead>
                <tbody>
                  {currentAudits.map((item, idx) => (
                    <tr key={item._id || idx} style={{ backgroundColor: idx % 2 === 0 ? '#ffffff' : '#faf5ff', borderBottom: '1px solid #e9d5ff' }}>
                      ...
                      <td style={{ padding: '14px 16px', color: '#1f2937', fontSize: '14px', wordBreak: 'break-word' }}>
                        <strong style={{ fontWeight: '700' }}>{item.name || 'N/A'}</strong>
                      </td>
                      <td style={{ padding: '14px 16px', color: '#1f2937', fontSize: '13px', wordBreak: 'break-all' }}>
                        {item.email}<br />
                        <small style={{ color: '#7928CA', fontWeight: '700', fontSize: '12px' }}>{item.phone || 'No phone'}</small>
                      </td>
                      <td style={{ padding: '14px 16px', color: '#1f2937', fontSize: '13px', wordBreak: 'break-all' }}>
                        {item.website ? (
                          <a href={item.website.startsWith('http') ? item.website : `https://${item.website}`} target="_blank" rel="noopener noreferrer" style={{ color: '#7928CA', textDecoration: 'underline' }}>
                            {item.website}
                          </a>
                        ) : (
                      ...
                      <td style={{ padding: '14px 16px', color: '#4b5563', fontSize: '13px', lineHeight: '1.5', wordBreak: 'break-word' }}>
                        {item.stuff || 'No context provided'}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', marginTop: '30px' }}>
                ...
                <span style={{ fontSize: '13px', color: '#6b7280', fontWeight: '500' }}>
                  Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, audits.length)} of {audits.length} entries
                </span>
              </div>
            )}
          </div>
